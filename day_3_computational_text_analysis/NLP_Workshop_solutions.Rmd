...
speeches <- function(n_speeches = 251) {
  ...
  data <- fread("sotu.tsv", sep = "\t", nrows = n_speeches)
...
tokenized_speeches <- 
  ...
  unnest_tokens("tokens", text, to_lower = TRUE) |>
...
```{r}
# Below, count the words and returns a list of words
...
words_plot <- words[order(-words$n), "n"]
...
i2w <- unique(tokenized_speeches$tokens)
w2i <- setNames(seq_along(i2w), i2w)
...
slice_max(tf, tf_idf, by = id) %>% 
  arrange(id)

#Least important word for each speech
slice_min(tf, tf_idf, by = id) %>% 
  arrange(id)
```
...
dtm_scratch <- build_dtm_from_scratch(tokens_df %>% filter(id %in% 1:4))
dtm_scratch[1:4, 1:4]
```
